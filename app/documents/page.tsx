...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-[var(--color-es-line)] bg-white px-6 py-16 text-center">
      <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-2xl bg-[#f1f0ec]">
        <FileText className="text-[#8a8a87]" />
      </div>
      <h2 className="font-display text-2xl font-semibold tracking-[-0.03em] text-[#2f3430]">
        No documents yet
      </h2>
      <p className="mx-auto mt-1.5 max-w-xs text-sm text-[#8a8a87]">
...
export default function DocumentsPage() {
  ...
  const ownerCount = rooms.filter((room) => room.role === "owner").length;
  const sharedCount = rooms.length - ownerCount;
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <Breadcrumb>
            <BreadcrumbList className="text-xs">
              <BreadcrumbItem>
                <BreadcrumbPage className="text-es-muted">Workspace</BreadcrumbPage>
              </BreadcrumbItem>
              <BreadcrumbItem>
                <BreadcrumbPage className="text-es-ink">Documents</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          <h1 className="mt-2 font-display text-[3rem] font-semibold leading-[0.98] tracking-[-0.045em] text-[#2f3430]">
            All Documents
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="icon"
            className="h-9 w-9 rounded-md border-[var(--color-es-line)] bg-white text-es-soft-ink"
          >
        ...
        <>
          <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
            {featured && (
              ...
              >
                <div className="mb-5 flex items-center gap-3">
                  <span className="rounded-full border border-[#d8d3c8] bg-white px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#5e5a53]">
                    In progress
                  </span>
                  <span className="text-xs text-[#8a8a87]">
                    {featured.document?.updatedAt
                      ? `Edited ${new Date(featured.document.updatedAt).toLocaleDateString(undefined, {
                          day: "numeric",
                          month: "short",
                        })}`
                      : "Recently updated"}
                ...
                <h2 className="max-w-2xl text-[2.2rem] font-semibold leading-[1.06] tracking-[-0.03em] text-[#2f3430]">
                  {featured.document?.title ?? "Untitled"}
                </h2>
                <p className="mt-3 max-w-xl text-sm leading-6 text-[#5e5a53]">
                  Continue writing and refining this draft with replay-ready edits and collaborative comments.
                </p>
                <div className="mt-8 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-[11px] text-[#8a8a87]">
            ...
            <div className="rounded-2xl border border-[#ece9e1] bg-[#f1f2ef] p-5">
              {secondary ? (
                <Link href={`/documents/${secondary.id}`} className="group block h-full">
                  <div className="mb-6 flex h-10 w-10 items-center justify-center rounded-lg bg-white text-[#6a6b66]">
                    <Sparkles className="h-4 w-4" />
                  </div>
                  <h3 className="text-[1.9rem] font-semibold leading-[1.07] tracking-[-0.03em] text-[#2f3430]">
                    {secondary.document?.title ?? "Untitled"}
                  </h3>
                  <p className="mt-2 text-sm text-[#6e6a63]">
                    {secondary.document?.updatedAt
                      ? `Last modified ${new Date(secondary.document.updatedAt).toLocaleDateString(undefined, {
                          day: "numeric",
                          month: "short",
                        })}`
                      : "Recently updated"}
                ...
                <div className="flex h-full flex-col items-center justify-center text-center">
                  ...
                  <p className="text-sm text-es-muted">No secondary document yet.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {gridRooms.map((room) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="line-clamp-2 min-h-[40px] text-base font-semibold leading-5 text-[#2f3430]">
                    {title}
                  </p>
                  <div className="mt-2 flex items-center justify-between text-[11px] text-[#8a8a87]">
            ...
            <div className="flex min-h-[248px] items-center justify-center rounded-xl border border-dashed border-[#d3d0c7] bg-[#faf9f6] p-4 text-center">
